...
export default function CheckoutModal({ open, onClose }) {
  ...
  return (
    <Modal open={open} onClose={close} title="Checkout" wide subtitle={step < 3 ? `Step ${step} of 2` : 'Order placed!'}>
      ...
      {step === 2 && (
        <div>
          <div className="fg"><label>Coupon Code</label><div style={{ display: 'flex', gap: 8 }}><input value={coupon} onChange={(e) => setCoupon(e.target.value.toUpperCase())} style={{ flex: 1 }} /><button type="button" className="btn btn-secondary" onClick={applyCoupon}>Apply</button></div></div>
