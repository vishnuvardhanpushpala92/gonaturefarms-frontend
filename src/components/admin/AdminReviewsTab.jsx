...
export default function AdminReviewsTab() {
  ...
  const act = async (id, action) => {
    const { data } = await api.put(`/admin/reviews/${id}/${action}`);
    showToast(data.message);
    load();
  };

  const feature = async (id, featured) => {
    const { data } = await api.put(`/admin/reviews/${id}/feature`, { featured });
    showToast(data.message);
    load();
  };
  ...
  const saveReview = async (e) => {
    ...
    try {
      if (editingReview) {
        const { data } = await api.put(`/admin/reviews/${editingReview.id}`, form);
        showToast(data.message);
      } else {
        const { data } = await api.post('/admin/reviews', form);
        showToast(data.message);
      }
        ...
        <div className="admin-card" style={{ marginBottom: 16 }}>
          ...
          <form onSubmit={saveReview}>
            <div className="fg">
              ...
              <input required value={form.productId} onChange={(e) => setForm({ ...form, productId: e.target.value })} placeholder="Enter product ID" />
            </div>
            <div className="fg">
              <label>Customer Name</label>
              <input required value={form.customerName} onChange={(e) => setForm({ ...form, customerName: e.target.value })} placeholder="Enter customer name" />
            </div>
            <div className="fg">
              <label>Rating</label>
            ...
            <div className="fg">
              <label>Comment</label>
              <textarea required value={form.comment} onChange={(e) => setForm({ ...form, comment: e.target.value })} placeholder="Enter review comment" />
            </div>
            <div className="fg">
              <label>
                <input type="checkbox" checked={form.featured} onChange={(e) => setForm({ ...form, featured: e.target.checked })} style={{ marginRight: 8 }} />
                Featured Review
              </label>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button type="submit" className="btn btn-primary">{editingReview ? 'Update Review' : 'Add Review'}</button>
