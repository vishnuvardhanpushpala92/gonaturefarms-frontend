...
export default function AdminReviewsTab() {
  ...
  const saveReview = async (e) => {
    ...
    try {
      ...
      setForm({ productId: '', customerName: '', rating: 5, comment: '', featured: false });
      load();
    } catch (err) {
      ...
      {showAddForm && (
        <div className="admin-card" style={{ marginBottom: 16 }}>
          ...
          <form onSubmit={saveReview}>
            <div className="fg"><label>Product ID</label><input required value={form.productId} onChange={(e) => setForm({ ...form, productId: e.target.value })} /></div>
            <div className="fg"><label>Customer Name</label><input required value={form.customerName} onChange={(e) => setForm({ ...form, customerName: e.target.value })} /></div>
            <div className="fg"><label>Rating</label><select value={form.rating} onChange={(e) => setForm({ ...form, rating: Number(e.target.value) })}>{[5, 4, 3, 2, 1].map((n) => <option key={n} value={n}>{n} ★</option>)}</select></div>
            <div className="fg"><label>Comment</label><textarea required value={form.comment} onChange={(e) => setForm({ ...form, comment: e.target.value })} /></div>
            <div className="fg"><label><input type="checkbox" checked={form.featured} onChange={(e) => setForm({ ...form, featured: e.target.checked })} style={{ marginRight: 8 }} />Featured Review</label></div>
            <div style={{ display: 'flex', gap: 8 }}><button type="submit" className="btn btn-primary">{editingReview ? 'Update Review' : 'Add Review'}</button><button type="button" className="btn btn-secondary" onClick={cancelEdit}>Cancel</button></div>
          </form>
        </div>
      )}
        ...
        <tbody>
          {reviews.map((r) => (
            <tr key={r.id}>
              ...
              <td>{r.customerName || r.userName}</td>
